...
describe("adsMath agent — deterministic forecaster", () => {
  ...
  it("produces three labeled scenarios", async () => {
    const res = await (adsMath.run as (c: typeof ctx, i: unknown) => ReturnType<typeof adsMath.run>)(ctx, { monthlyBudget: 5000 });
  ...
  it("ROAS scales linearly with the multiplier", async () => {
    const res = await (adsMath.run as (c: typeof ctx, i: unknown) => ReturnType<typeof adsMath.run>)(ctx, { monthlyBudget: 1000 });
    expect(res.ok).toBe(true);
  ...
  it("spend is invariant across scenarios (budget is fixed)", async () => {
    const res = await (adsMath.run as (c: typeof ctx, i: unknown) => ReturnType<typeof adsMath.run>)(ctx, { monthlyBudget: 7500 });
    if (!res.ok || !res.output) return;
  ...
  it("validates negative budget", () => {
    expect(() =>
      (adsMath.validate as unknown as (i: { monthlyBudget: number }) => void)({
        monthlyBudget: -1,
      }),
    ).toThrow();
  });
